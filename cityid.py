...
city_S = {"10101":"北京","10102":"上海","10103":"天津","10104":"重庆","10105":"黑龙江","10106":"吉林",
 "10107":"辽宁","10108":"内蒙古","10109":"河北","10110":"山西","10111":"陕西","10112":"山东",
 "10113":"新疆","10114":"西藏","10115":"青海","10116":"甘肃","10117":"宁夏","10118":"河南",
 "10119":"江苏","10120":"湖北","10121":"浙江","10122":"安徽","10123":"福建","10124":"江西",
 "10125":"湖南","10126":"贵州","10127":"四川","10128":"广东","10129":"云南","10130":"广西",
...
def main(x,x1,x2):
    if x1 == '' and x2 == '':
        CityId = city_X[x][list(city_X[x].keys())[0]][list(city_X[x].keys())[0]]["AREAID"]
    elif x1 != '' and x2 == '': 
        CityId = city_X[x][x1][x1]["AREAID"]
    elif city_X[x][x1][x2]["NAMECN"] == x2:
        CityId = city_X[x][x1][x2]["AREAID"]
    url = f'http://www.weather.com.cn/weather1d/{CityId}.shtml'
    url_7, url_8 = get_url(url)
    data_1 = date7(url_7)
    forecast7Days = pd.DataFrame(data_1).set_index('日期')
    # forecast7Days['最高气温'] = forecast7Days['最高气温'].map(lambda x:x+'℃')
    data_2 = date8_15(url_8)
    forecast8Days=pd.DataFrame(data_2).set_index('日期')
    forecast8Days.index = forecast8Days.index.map(lambda x:''.join(re.findall('[0-9]{1,2}',x[3:6])) + '日' + '（' + x[0:2]+'）')
    forecast8Days["最低气温"] = forecast8Days["最低气温"].str.replace("/", "")
    data_all = pd.concat([forecast7Days, forecast8Days], axis=0)
    
    
    return forecast7Days,data_all
